# Git Team-1: Branch + Commit (Single Author, No Co-author)
...
- **Frontend (FE) changes** → `yamone <yamone@lomtech.net>`
- **Backend (BE) changes** → `kyawyelin <kyawyelin@lomtech.net>`
...
4. **Commit with correct author only (no Co-author)**
   - **FE-only:**  
     `git commit -m "<message>" --author="yamone <yamone@lomtech.net>"`
   - **BE-only:**  
     `git commit -m "<message>" --author="kyawyelin <kyawyelin@lomtech.net>"`
   - **Both FE and BE:**  
     - Option A: Two commits.  
       - Stage only FE paths → `git commit -m "<fe message>" --author="yamone <yamone@lomtech.net>"`  
       - Stage remaining BE paths → `git commit -m "<be message>" --author="kyawyelin <kyawyelin@lomtech.net>"`
...
- **Author only:** Use `--author="Name <email>"` so the commit shows only the chosen team email (yamone for FE, kyawyelin for BE).
...
3. Stage and commit using only `--author="yamone <yamone@lomtech.net>"` for FE or `--author="kyawyelin <kyawyelin@lomtech.net>"` for BE (split commits if both).
