# Git Team-2: Branch + Commit (Single Author, No Co-author)
...
- **Frontend (FE) changes** → `shinkhant <shinkhant@lomtech.net>`
- **Backend (BE) changes** → `aungyehtet <aungyehtet@lomtech.net>`

Use only these authors. Do not add any `Co-authored-by` trailer. **Do not run or rely on `git config user.name` or `git config user.email`**—the current machine's Git login must never be read, logged, or used for the commit author.
...
4. **Commit with correct author only (no Co-author)**
   - **FE-only:**  
     `git commit -m "<message>" --author="shinkhant <shinkhant@lomtech.net>"`
   - **BE-only:**  
     `git commit -m "<message>" --author="aungyehtet <aungyehtet@lomtech.net>"`
   - **Both FE and BE:**  
     - Option A: Two commits.  
       - Stage only FE paths → `git commit -m "<fe message>" --author="shinkhant <shinkhant@lomtech.net>"`  
       - Stage remaining BE paths → `git commit -m "<be message>" --author="aungyehtet <aungyehtet@lomtech.net>"`
     - Option B: Single commit with one author only if changes are tightly coupled; choose FE or BE author by which part is dominant.
...
- **Author only:** Use `--author="Name <email>"` so the commit shows only the chosen team email (shinkhant for FE, aungyehtet for BE).
- **Never expose local Git identity:** Do not run `git config --get user.email` or `user.name`; do not use, display, or log the current machine's Git login. Use only the `--author=` strings from the Author Mapping above.
...
3. Stage and commit using only `--author="shinkhant <shinkhant@lomtech.net>"` for FE or `--author="aungyehtet <aungyehtet@lomtech.net>"` for BE (split commits if both).
4. No Co-authored-by, no Cursor attribution. Never read or use this machine's Git user/email—only the team authors listed in this file.
