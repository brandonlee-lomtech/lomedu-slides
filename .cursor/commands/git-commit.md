...
4. **Create short commit message**
    ...
    - Use only `-m "..."` with a single-line message (no commit body, no trailers)
...
- **No co-author:** Use only `git commit -m "..."` with a single-line message. Do not add `Co-authored-by`, `Signed-off-by`, or any other trailer to the commit. No commit body—subject line only.